...
public class MusicDemo1 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int k = 0; k < chords.length; k++) {
				...
				if(randomInt==0)
				{
					...
					guitar.add(melodyGenerateUtil.getMelody(chords[k]));
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int k = 0; k < chords.length; k++) {
				...
				else if(randomInt==3)
				{
					...
					drum.add("C2h");
					drum.add("C2h");
					violin.add(melodyGenerateUtil.getMelody(chords[k]));
				}
		...
		playThread.start();
		MusicSaveThread saveThread=new MusicSaveThread(song, "beiDuoFen1");
		saveThread.start();
