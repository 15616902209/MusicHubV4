...
public class BasePlayTest7 {
    ...
    public static void main(String[] args) {
        ...
        chordPattern.setVoice(0).setInstrument(Instruments.Piano).setTempo(96);
        Pattern melodyPattern =new Pattern();
        melodyPattern.setVoice(1).setInstrument(Instruments.Piano).setTempo(96);

        List<Chord> chordList =new ArrayList<>();

        Chord chordParam =new Chord(new Note("C"),Chord.MAJOR_SEVENTH_INTERVALS);
        chordList.add(chordParam);
        chordParam =new Chord(new Note("A"),Chord.MINOR_SEVENTH_INTERVALS);
        chordList.add(chordParam);
        chordParam =new Chord(new Note("F"),Chord.MAJOR_SEVENTH_INTERVALS);
        chordList.add(chordParam);
        chordParam =new Chord(new Note("G"),Chord.MAJOR_SEVENTH_INTERVALS);
        chordList.add(chordParam);
