...
/**
 * 作者： SachZhong 钟盛勤 项目名：MusicHub 版本： 1.0 创建时间：2020年1月15日 下午6:07:43
 * 类名：GenerateIntro.java 类说明：生成前奏
 */
public class MusicIntroImpl implements AGenerateMusic {
	...
	public List<Pattern> generateMusic(String jidiao, String chordpath, int yinyu, int speed) {
		...
		for (int k = 0; k < chords.length; k++) {
			...
			if (randomInt == 0) {
				...
				pianoMelody.add(melodyGenerateUtil.getMelody(chords[k], yinyu + 3));
