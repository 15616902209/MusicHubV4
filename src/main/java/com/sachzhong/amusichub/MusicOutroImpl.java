...
import com.sachzhong.util.EndRandomGenerateUtil2;
...
/**
 * 作者： SachZhong 钟盛勤 项目名：MusicHub 版本： 1.0 创建时间：2020年1月15日 下午6:12:40
 * 类名：GenerateOutro.java 类说明：生成结尾
 */
public class MusicOutroImpl implements AGenerateMusic {
	...
	public List<Pattern> generateMusic(String jidiao, String chordpath, int yinyu, int speed) {
		...
		// 结束音生成工具
		EndRandomGenerateUtil2 endRandomGenerateUtil2 = new EndRandomGenerateUtil2();
		...
		for (int k = 0; k < chords.length; k++) {
			...
			if (randomInt == 0) {
				pianoMelody.add(endRandomGenerateUtil2.getEnd(chords[k]));
